'''
Test all the features of CAMPInterestClassifier class with poor HMM classifier.
'''
...
# Define object to predict Object 1 as
o = 2
...
for file in keep_files_train:
    ...
    # Assume HMM predicts correct object perfectly unless object 1 
...
# Save trained model
data_file = os.path.join(save_data, 'trained_conf_obj2.pkl')
with open(data_file, 'wb') as handle:
    pickle.dump(obj, handle, protocol=pickle.HIGHEST_PROTOCOL)

# Place to save figures
save_fig = os.path.join(pwd, 'figs', 'bad_class', 'high_conf')
...
plt.ylabel('P(I=1)')
plt.title('Interest By Object Type with Object 1 Predicted as Object 2')
plt.grid()
...
# Look at learning for Object 1 predicted as Object 2
obj = [2]
for o in obj:
    ...
    file_name = 'learning_rate_obj1_obj2.png'
    fig = plt.figure(figsize = (15, 10))
    ...
    plt.title('Interest by Epoch with Object 1 Predicted as Object 2')
    plt.grid()
    plt.legend()
    plt.xticks(epochs)
    plt.savefig(os.path.join(save_fig, file_name), dpi=150)

# Define all object types to predict Object 1 as
obj_type = [1, 2, 3, 4, 5, 6]

# Initialize dict to store
obj_type_dict = {ot: {} for ot in obj_type}
...
for o in obj_type:
    ...
    # Now run 20 files through to predict interest
    stored_interest = np.zeros(shape=(n_each, len(targets)))
    counter = np.zeros(len(targets), dtype=int)
    for file in keep_files_test:
    ...
    # Get average across all 20 tracks and store in dict
    obj_type_dict[o] = np.mean(stored_interest, axis=0)

# Print out dictionary
for o in obj_type:
    print(obj_type_dict[o])
    
with open(os.path.join(save_fig, 'obj_type_dict.csv') ,'w') as f:
    writer = csv_py.writer(f, delimiter=',',lineterminator='\n')
    for o in obj_type:
        row = obj_type_dict[o]
        for i in range(len(row)):
            row[i] = np.format_float_positional(row[i], precision=3)
        writer.writerow(row)
