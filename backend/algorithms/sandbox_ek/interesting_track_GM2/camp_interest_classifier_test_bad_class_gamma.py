...
# Define gamma for uniform classifier
gamma = 1 / len(targets)
...
# Save trained model
data_file = os.path.join(save_data, 'trained_gamma_uniform.pkl')
with open(data_file, 'wb') as handle:
    pickle.dump(obj, handle, protocol=pickle.HIGHEST_PROTOCOL)

# Place to save figures
save_fig = os.path.join(pwd, 'figs', 'bad_class', 'gamma')
...
plt.ylabel('P(I=1)')
plt.title('Interest By Object Type with $\gamma=0.167$')
plt.grid()
...
# Look at learning for different values of gamma
gamma = [0.167]
for g in gamma:
    ...
    for file in keep_files_epoch:
        ...
        if start_flag:
            ...
            for file_test in keep_files_test:
                ...
                # Assume HMM predicts correct object as gamma% chance
                p_o_given_x = np.ones(len(targets)) * (1 - g) / (len(targets) - 1)
    ...
    file_name = 'learning_rate_gamma_point1.png'
    fig = plt.figure(figsize = (15, 10))
    ...
    plt.title('Interest by Epoch with $\gamma=0.167$')
    plt.grid()
    plt.legend()
    plt.xticks(epochs)
    plt.savefig(os.path.join(save_fig, file_name), dpi=150)

# Loop through to create table for different gamma values

# Define all gamma values to test
gamma = [1, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1, 0.]

# Initialize dict to store
gamma_dict = {gam: {} for gam in gamma}
...
for g in gamma:
    ...
    gamma_dict[g] = np.mean(stored_interest, axis=0)
    
with open(os.path.join(save_fig, 'gamma_dict.csv') ,'w') as f:
