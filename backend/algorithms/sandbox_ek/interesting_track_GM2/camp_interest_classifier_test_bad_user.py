'''
Test all the features of CAMPInterestClassifier class with bad user inputs.
'''
...
# Disinterested in Object 1, otherwise interested
u_o = [-1, 1, 1, 1, 1, 1]

# Disinterested in all geography
u_g = [-1, -1, -1, -1, -1, -1, -1, -1]
...
# Save trained model
data_file = os.path.join(save_data, 'trained_bad_user.pkl')
with open(data_file, 'wb') as handle:
    pickle.dump(obj, handle, protocol=pickle.HIGHEST_PROTOCOL)

# Place to save figures
save_fig = os.path.join(pwd, 'figs', 'bad_user')
...
plt.ylabel('P(I=1)')
plt.title('Interest By Object Type with Poor User Inputs')
plt.grid()
...
plt.title('Interest by Epoch with Poor User Inputs')
plt.grid()
plt.legend()
plt.xticks(epochs)
plt.savefig(os.path.join(save_fig, file_name), dpi=150)
